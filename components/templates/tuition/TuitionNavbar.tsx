...
export function TuitionNavbar({ config }: { config: TuitionConfig }) {
    ...
    <header className={cn("fixed top-0 z-50 w-full transition-all duration-300",
      ...
      <nav className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-1 md:flex">
          {navLinks.map((link) => (
            <a key={link.href} href={link.href}
              className={cn("px-3 py-2 text-sm font-medium transition-colors hover:text-blue-500", scrolled ? "text-foreground/70" : "text-white/80")}>
              {link.label}
            </a>
          ))}
        </div>
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <Button asChild size="sm" className="hidden bg-blue-600 text-white hover:bg-blue-700 md:flex">
              ...
              <nav className="flex flex-col gap-2">
                {navLinks.map((link) => (
                  <a key={link.href} href={link.href} onClick={() => setOpen(false)}
                    className="rounded-md px-4 py-3 text-sm hover:bg-muted hover:text-blue-500 transition-colors">{link.label}</a>
                ))}
                <Button className="mt-4 bg-blue-600 hover:bg-blue-700" onClick={() => setOpen(false)}>Register Now</Button>
              </nav>
            </SheetContent>
          </Sheet>
        </div>
